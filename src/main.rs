...
use auth::{ensure_mcp_request, require_bearer, BearerToken};
...
async fn run_http() -> Result<()> {
    ...
    // Bearer auth + request shims wrap /mcp only. /health stays open for Railway.
    // Outer layers run first: auth → Accept/body normalize → rmcp.
    let protected_mcp = Router::new()
        .fallback_service(mcp)
        .layer(middleware::from_fn(ensure_mcp_request))
